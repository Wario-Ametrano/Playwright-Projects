...
setup('autanticate by UI', async ({page},testInfo) => { //nome del test
    ...
    await page.goto('https://practicetestautomation.com/practice-test-login/'); //pagina di login
    await page.locator('#username').fill(user); //inserisco username trovandolo con il locator id
    await page.locator('#password').fill(password); //inserisco password
    await page.locator('#submit').click(); //clicco su login

    await page.waitForURL('https://practicetestautomation.com/logged-in-successfully/'); //aspetto che la pagina cambi
